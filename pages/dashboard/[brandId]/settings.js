...
import { useUser, deleteBrand, disableEmails } from '@/utils/useUser';
...
export default function BrandSettingsPage() {
  ...
  const handleDelete = async () => {
    if (window.confirm('Are you sure you want to delete this brand?')){
      await deleteBrand(router?.query?.brandId).then((result) => {
        if(result === "success"){
  ...
  const handleDisableEmails = async (type) => {    
    await disableEmails(router?.query?.brandId, type).then((result) => {
      if(result === "success"){
    ...
    <>
      ...
      <div className="pb-10 mb-12 border-b-4">
        <div className="pt-10 wrapper">
          <h1 className="text-2xl sm:text-3xl tracking-tight font-extrabold">Brand Settings</h1>
        </div>
      </div>
      <div className="wrapper">
        <div className="bg-white shadow-lg rounded-xl mt-5 max-w-3xl border-4 border-gray-200">
          <div className="p-6 sm:p-8">
            <h3 className="text-lg leading-6 font-medium text-gray-900">Brand ID</h3>
            <div className="mt-2 max-w-2xl text-gray-500">
              <p>{router?.query?.brandId}</p>
            </div>
          </div>
        </div>
        <div className="bg-white shadow-lg rounded-xl mt-5 max-w-3xl border-4 border-gray-200">
          <div className="p-6 sm:p-8">
            ...
            <div className="mt-2 max-w-2xl text-gray-500">
              <p>When enabled, you will receive email notifications for this brand whenever you receive a new submission.</p>
            </div>
            <div className="mt-5">
              <Switch
                checked={!activeBrand?.disable_emails}
                onChange={e=>{handleDisableEmails(activeBrand?.disable_emails ? false  : true)}}
                className={classNames(
                  !activeBrand?.disable_emails ? 'bg-green-600' : 'bg-red-600',
              ...
              >
                ...
                <span
                  aria-hidden="true"
                  className={classNames(
                    !activeBrand?.disable_emails ? 'translate-x-5' : 'translate-x-0',
        ...
        <div className="bg-white shadow-lg rounded-xl mt-6 max-w-3xl border-4 border-gray-200">
          <div className="p-6 sm:p-8">
            ...
            <div className="mt-2 max-w-2xl text-gray-500">
              <p>Once you delete your brand, you will lose all data, including all submissions and brand settings associated with it.</p>
            </div>
            <div className="mt-5">
              <button
